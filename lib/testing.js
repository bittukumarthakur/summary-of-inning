...
const isObjectLengthSame = function(firstObject, secondObject) {
  return Object.keys(firstObject).length === Object.keys(secondObject).length;
};

const isObjectSame = function(firstObject, secondObject) {
  if(!isObjectLengthSame(firstObject, secondObject)) {
    return false
  }
...
const assertObject = function(expected, actual, message) {
  display(assertTest(expected, actual, message, isObjectSame));
};
